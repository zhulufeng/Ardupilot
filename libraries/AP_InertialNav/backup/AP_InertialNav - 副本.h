...
#define AP_INTERTIALNAV_RNG_TIMEOUT_MS              2000     // timeout after which position error from GPS will fall to zero
...
public:
    ...
    // Constructor
    AP_InertialNav(AP_AHRS &ahrs, AP_Baro &baro, GPS_Glitch& gps_glitch, Baro_Glitch &baro_glitch,RangeFinder &sonar) :
        _ahrs(ahrs),
        _baro(baro),
        _sonar(sonar),
        _xy_enabled(false),
...
protected:
    ...
    void check_rng_glitch();
    void correct_with_rng(float rng_alt, float dt);
    float get_rng_climb_rate(void);
    void rng_inav_check();
    ...
    bool _flags_rng_initialised;
    bool _flags_rng_glitching,_flags_rng_glitching_internal,_flags_flow_glitching,_flags_flow_glitching_internal,_flags_flow_timeout;
